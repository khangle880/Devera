...
import 'package:icon/screens/auth/pin_security/pin_security_bloc.dart';
...
class PinSecurityView extends StatefulWidget {
  const PinSecurityView({Key? key}) : super(key: key);

  @override
  _PinSecurityViewState createState() => _PinSecurityViewState();
}

class _PinSecurityViewState extends State<PinSecurityView> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      create: (context) => PinSecurityBloc(
  ...
  void pinIndexSetup(String text, BuildContext context) {
    ...
    if (pinIndex == 4) {
      if (strPin == context.read<AuthCubit>().credentials!.pin) {
        context.read<PinSecurityBloc>().add(PinSecuritySubmitted(pin: strPin));
      } else {
        ExtensionFunction.showSnackBar(context, 'Pin is not corrected');
      }
    }
  }
  ...
  Widget _buildSecurityText() {
    return Text('Please enter your PIN to proceed',
