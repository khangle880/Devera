...
class User extends Model {
  ...
  const User._internal({required this.id, required username, email, avatarKey, description, quickNotes, checkLists, tasks}): _username = username, _email = email, _avatarKey = avatarKey, _description = description, _quickNotes = quickNotes, _checkLists = checkLists, _tasks = tasks;
  
  factory User({String? id, required String username, String? email, String? avatarKey, String? description, List<QuickNote>? quickNotes, List<CheckList>? checkLists, List<Task>? tasks}) {
    return User._internal(
      ...
      quickNotes: quickNotes != null ? List<QuickNote>.unmodifiable(quickNotes) : quickNotes,
      checkLists: checkLists != null ? List<CheckList>.unmodifiable(checkLists) : checkLists,
  ...
  User copyWith({String? id, String? username, String? email, String? avatarKey, String? description, List<QuickNote>? quickNotes, List<CheckList>? checkLists, List<Task>? tasks}) {
    return User(
      id: id ?? this.id,
    ...
    : id = json['id'],
      ...
      _quickNotes = json['quickNotes'] is List
        ? (json['quickNotes'] as List)
          ...
          .map((e) => QuickNote.fromJson(new Map<String, dynamic>.from(e['serializedData'])))
          .toList()
        : null,
      _checkLists = json['checkLists'] is List
        ? (json['checkLists'] as List)
          .where((e) => e?['serializedData'] != null)
          .map((e) => CheckList.fromJson(new Map<String, dynamic>.from(e['serializedData'])))
          .toList()
        : null,
  ...
  Map<String, dynamic> toJson() => {
    'id': id, 'username': _username, 'email': _email, 'avatarKey': _avatarKey, 'description': _description, 'quickNotes': _quickNotes?.map((e) => e?.toJson())?.toList(), 'checkLists': _checkLists?.map((e) => e?.toJson())?.toList(), 'tasks': _tasks?.map((e) => e?.toJson())?.toList()
  };
